...
export function storyIMF1997(engine, solveCase) {
  ...
  function street() {
    ...
    const found = e.addClue('c_chaos', '금융 공황의 시작', '국민은 절망에 빠졌으나, 상류층은 이미 대피 준비를 마친 듯했다.');
  ...
  function newspaper() {
    ...
    const found = e.addClue('c_gap', '발행과 실제의 괴리', '정부는 실제 수치를 알고 있었음에도 무책임한 낙관론으로 국민을 기만했다.');
  ...
  function stranger() {
    ...
    const found = e.addClue('c_layoff', '기획된 해고', '공식 발표 전 이미 기업들은 대규모 구조조정을 확정하고 있었다.');
  ...
  function stranger_talk() {
    ...
    const found = e.addClue('c_insider', '내부자 정보 유출', '고위층은 발표 전 정보를 공유하며 달러를 대거 매집했다.');
  ...
  function radio() {
    ...
    const found = e.addClue('c_radio', '중단된 보도', '정부 친인척의 달러 투기 의혹 보도가 외부 압력에 의해 강제 중단되었다.');
  ...
  function trashCan() {
    ...
    const found = e.addClue('c_transfer', '해외 자산 도피', '국가 부도 선언 직전, 고위직의 자산이 대규모로 해외로 빠져나간 증거.');
  ...
  function suit() {
    ...
    const found = e.addClue('c_party', '권력의 축제', '국가의 위기를 개인의 부를 축적할 기회로 삼은 이들의 소름 끼치는 대화.');
  ...
  function findK() {
    ...
    const found = e.addClue('c_k', 'K의 기밀 리포트', '72시간 동안 진행된 국가 자금의 불법 이전과 정보 유출의 총집 대성 기록.');
