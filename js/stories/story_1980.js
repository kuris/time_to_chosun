...
export function story1980(engine, solveCase) {
  ...
  function street() {
    ...
    const found = e.addClue('c_army_arrival', '군부대 전개', '이것은 단순한 치안 유지가 아닌, 점령군과 같은 대규모 무력 전개였다.');
  ...
  function wall() {
    ...
    const found = e.addClue('c_wall', '시민의 벽보', '신문이 침묵할 때, 시민들은 담벼락을 신문삼아 진실을 전하고 있었다.');
  ...
  function closer() {
    ...
    const found = e.addClue('c_suppress', '진압의 실체', '계엄군은 무고한 시민들을 향해 전쟁과 같은 폭력을 행사하고 있었다.');
  ...
  function citizen() {
    ...
    const found = e.addClue('c_isolated', '고립된 광주', '외부로 향하는 모든 통신과 교통이 계엄군에 의해 완벽히 차단되었다.');
  ...
  function journalist() {
    ...
    const found = e.addClue('c_film', '기자의 필름', '사망자 발생과 실탄 지급 사실이 담긴 결정적 증거 필름.');
  ...
  function mysteryMan() {
    ...
    const found = e.addClue('c_doc', '계엄사 작전 지침', '사망자가 발생해도 좋다는 식의 잔인한 무력 진압 명령이 기재된 문건.');
